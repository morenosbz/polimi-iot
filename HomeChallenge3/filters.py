...
def question4():
	...
	conn_print = lambda r: r[IP].dst + ':' + str(r[TCP].sport)
	...
	f4 = lambda r: MQTT in r and r[MQTT].type == 3 and "factory/department" in r[MQTT].topic
	topic_pkts = hw.filter(f4)
	print "Packets with the factory/department topic = " + str(len(topic_pkts))
	
	f42 = lambda r: r[TCP].sport in connect_ports
	jane_publish = topic_pkts.filter(f42)
